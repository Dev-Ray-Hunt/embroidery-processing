...
def render_png(dst_path: Path) -> bytes:
    ...
    pyembroidery.write_png writes to a file path (no in-memory stream API), so
    we round-trip through a temp file created with mkstemp (no insecure-temp
    race). The temp file is cleaned up before returning regardless of success.
    """
